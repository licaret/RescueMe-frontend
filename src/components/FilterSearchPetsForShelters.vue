<template>
      ...
      >
        <div v-if="showFilters" class="px-6 py-5 border-t border-gray-100 bg-gray-50">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            ...
            <!-- Checkboxes and Clear Filters Button -->
  ...
  import { getBreedsBySpecies } from '@/services/pet_service';
  
  export default {
    name: 'FilterSearchPetsForShelters',
    props: {
      initialFilters: {
        type: Object,
        default: () => ({})
    ...
    emits: ['filter-changed', 'sort-changed', 'reset-filters'],
    
    setup(props, { emit }) {

      const filters = ref({
        name: props.initialFilters.name || "",
        species: props.initialFilters.species || "",
        breed: props.initialFilters.breed || "",
        sex: props.initialFilters.sex || "",
        size: props.initialFilters.size || "",
        healthStatus: props.initialFilters.healthStatus || "",
        vaccinated: props.initialFilters.vaccinated || "",
        neutered: props.initialFilters.neutered || "",
        urgentAdoptionNeeded: props.initialFilters.urgentAdoptionNeeded || "",
        timeSpentInShelter: props.initialFilters.timeSpentInShelter || "",
        status: props.initialFilters.status || "",
        ageRange: props.initialFilters.ageRange || ""
      });
      ...
      const fetchBreeds = async () => {
        ...
        try {
            const breeds = await getBreedsBySpecies(filters.value.species);
            breedOptions.value = breeds;
        } catch (error) {
